"""Web tools: web_search and web_fetch with SSRF defense."""
    ...
    async def execute(self, query: str, count: int | None = None, **kwargs: Any) -> str:
        if not self.api_key:
            return (
                ...
                "~/.companio/config.json under tools.web.search.apiKey "
...
class WebFetchTool(Tool):
    ...
    async def execute(
        ...
        # SSRF defense: block requests to internal/private IP addresses
        parsed = urlparse(url)
        hostname = parsed.hostname or ""
        if _is_internal_ip(hostname):
            return json.dumps(
                {"error": "Blocked: URL resolves to an internal/private IP address", "url": url},
                ensure_ascii=False,
            )
