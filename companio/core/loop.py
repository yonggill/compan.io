...
from companio.core.subagent import SubagentManager
from companio.tools.cron_tool import CronTool
from companio.tools.filesystem import EditFileTool, ListDirTool, ReadFileTool, WriteFileTool
...
from companio.tools.spawn import SpawnTool
from companio.tools.web import WebFetchTool, WebSearchTool
...
if TYPE_CHECKING:
    from companio.config.schema import ChannelsConfig, ExecToolConfig
...
class AgentLoop:
    ...
    def __init__(
        ...
        from companio.config.schema import ExecToolConfig
    ...
    def _register_default_tools(self) -> None:
        ...
        if self.obsidian_vault_path:
            self.tools.register(ObsidianTool(vault_path=self.obsidian_vault_path))
    ...
    async def _connect_mcp(self) -> None:
        ...
        from companio.tools.mcp import connect_mcp_servers
    ...
    async def _process_message(
        ...
        if cmd == "/new":
            lock = self._consolidation_locks.setdefault(session.session_id, asyncio.Lock())
            self._consolidating.add(session.session_id)
            try:
                async with lock:
                    snapshot = session.messages[session.last_consolidated :]
                    if snapshot:
                        temp = Session(session_id=session.session_id)
        ...
        history = session.messages[-self.memory_window :]
        # Trim orphan tool results at the start caused by window slicing
